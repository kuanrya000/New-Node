...
class MainScreen:
    def __init__(self, root):
        ...
        # Points to a single syllable
        self.syllable_pointer = syllables.Syllable()
        ...
        # Sliders for each syllable
        self.vol_slider = Scale(self.f2, orient=HORIZONTAL, label='Volume', length=175, from_=-50, to=50, width=22,
                           command = self.syllable_pointer.ChangeVolume)

        self.pitch_slider = Scale(self.f2, orient=HORIZONTAL, from_=-50, to=50, label='Pitch', length=175, width=22,
                             command = self.syllable_pointer.ChangePitch)

        self.speed_slider = Scale(self.f2, orient=HORIZONTAL, label='Speed', from_=-50, to=50, length=175, width=22,
                             command = self.syllable_pointer.ChangeLength)
    ...
    def play_sound(self, num):
        self.sampler.play(self.syllables_audio_list[num].Activate())

    # Plays all sounds
    def play_sounds(self):
        start_time = time.time()
        for syllable in self.syllables_audio_list:
            if ((time.time() - start_time) >= syllable.start_time):
                self.sampler.play(syllable.Activate())

    # After clicking on a syllable button
    def SetSliderVal(self):
        self.vol_slider.set(self.syllable_pointer.volume)
        self.speed_slider.set(self.syllable_pointer.length)
        self.pitch_slider.set(self.syllable_pointer.pitch)
        return

    def CreateButtons(self):
        x = 0
        for x in range(0, len(self.syllables_audio_list)):
            self.buttons_list.append(Button(self.Tops, bd=8, fg="black", font=('arial', 20, 'bold'), text = self.syllables_audio_list[x].audio_name,
                                            bg="powder blue", command=lambda: self.play_sound(x), height = 5, width = 10))
            self.buttons_list[x].grid(row = 0, column = x)
    ...
    def RemakeSyllables(self, string):
        ...
        if (self.syllables_audio_list):
            self.syllable_pointer = self.syllables_audio_list[0]
            self.SetSliderVal()
